...
window.onload = function initCube() {
	...
	fsLoc = gl.getUniformLocation(program, "fs");
	gl.uniform3fv(fsLoc, fs);
	...
	document.getElementById("xfsbutton").onchange = function ( event ) {
	   
		var speedx = event.target.value;
	    sfaxis = xAxis;
		fs[sfaxis] = speedx;
	}
	
	document.getElementById("yfsbutton").onchange = function (event) {
	    
		var speedy = event.target.value;
	    sfaxis = yAxis;
		fs[sfaxis] = speedy;
	}
	
	document.getElementById("zfsbutton").onchange = function (event) {
		 
		var speedz = event.target.value;
	    sfaxis = zAxis;
		fs[sfaxis] = speedz;
	}
